...
from NeonV2_uup_fxe_fxy_trade_log
group by yr, mn order by yr, mn;

drop view if exists trades;
create view trades as
select A.tm, A.worth/B.worth - 1 as ret
from NeonV2_uup_fxe_fxy_trade_log as A join NeonV2_uup_fxe_fxy_trade_log as B
on A.tm = (
   select tm from NeonV2_uup_fxe_fxy_trade_log where tm > B.tm
...
from
NeonV2_uup_fxe_fxy_trade_log as A join
NeonV2_uup_fxe_fxy_trade_log as B on A.tm >= B.tm
group by A.tm
);
...
select B.tm,
-- (abs(B.uup_q - A.uup_q) + abs(B.fxe_q - A.fxe_q) + abs(B.fxy_q - A.fxy_q))/(abs(A.uup_q) + abs(A.fxe_q) + abs(A.fxy_q))
(abs(B.uup_q - A.uup_q) + abs(B.fxe_q - A.fxe_q) + abs(B.fxy_q - A.fxy_q))/A.worth
as ratio
from NeonV2_uup_fxe_fxy_trade_log as A join NeonV2_uup_fxe_fxy_trade_log as B
on B.tm = (
   select tm from NeonV2_uup_fxe_fxy_trade_log where tm > A.tm limit 1
)
...
select avg(exposure) from (
       select tm, (abs(uup_q) + abs(fxe_q) + abs(fxy_q))/worth as exposure
       from NeonV2_uup_fxe_fxy_trade_log
--       where tm < '2016-06-23'
) as tbl;
